...
impl Session {
    pub fn new(address: &str) -> Session {
        Session { endpoint: format!("http://{}/v1/session", address) }
    }
        ...
        for _ in 0..10 {
            ...
            if resp.get_code() != 200 {
                if resp.get_code() == 404 {
                    println!("Could not renew session: {}, returned HTTP code: {:?}. Returning \
                              false.",
                             session_id,
                             resp.get_code());
                    return false;
                } else {
                    println!("Could not renew session: {}, returned HTTP code: {:?}. Sleeping \
                              for 2 seconds",
                             session_id,
                             resp.get_code());
